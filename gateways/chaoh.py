...
def categorize_chaos_response(api_response):
    """Categorize Chaos API response"""
    try:
...
def format_chaos_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format Chaos API response for Telegram"""
...
class ChaosGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/chaos/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through Chaos API"""
...
<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /st cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "Chaos API", message):
            return
        ...
        # Process the card using Chaos Gateway
        ...
        if error:
            ...
            response_text = format_chaos_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_chaos_response(api_response)
    ...
    print("✅ Chaos Gateway commands registered")
