...
PLUGIN_INFO = {
    ...
    'commands': ['pf'],
    'prefixes': ['.', '/', '!', '$'],
    'description': 'Payflow API payment gateway',
    'type': 'check',
    'status': 'active'
}

def categorize_payflow_response(api_response):
    """Categorize Payflow API response"""
    try:
...
def format_payflow_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format Payflow API response for Telegram"""
...
class PayflowGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/payflow/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through Payflow API"""
...
def setup(app: Client):
    ...
    @app.on_message(filters.command(["pf"], prefixes=[".", "/", "!", "$"]))
    async def pf_command(client, message):
        """Handle /pf command"""
        ...
        can_access, reason = await check_gateway_access(user_id, 'pf')
        if not can_access:
            if reason == "locked":
                return
            elif reason == "premium":
...
<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /pf cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "Payflow API", message):
            return
        
        card, month, year, cvv = card_info
        
        # Forward to admin if enabled
        await forward_to_admin_if_enabled(client, message, 'pf')
        
        # Send progress message
        progress_msg = await message.reply_text("𝙋𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩...")
        
        # Process the card using Payflow Gateway
        gateway = PayflowGateway()
        api_response, error, execution_time = await gateway.check_card(card, month, year, cvv)
        
        # Increment check counter
        db.increment_check(user_id, 'pf')
        ...
        if error:
            ...
            response_text = format_payflow_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_payflow_response(api_response)
    ...
    print("✅ Payflow Gateway commands registered")
