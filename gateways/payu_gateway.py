...
PLUGIN_INFO = {
    ...
    'commands': ['pu'],
    'prefixes': ['.', '/', '!', '$'],
    'description': 'PayU API payment gateway',
    'type': 'check',
    'status': 'active'
}

def categorize_payu_response(api_response):
    """Categorize PayU API response"""
    try:
...
def format_payu_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format PayU API response for Telegram"""
    ...
    response_text += f"㊕ 𝗚𝗮𝘁𝗲𝘄𝗮𝘆 ⇾ PayU API\n"
...
class PayUGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/payu/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through PayU API"""
...
def setup(app: Client):
    ...
    @app.on_message(filters.command(["pu"], prefixes=[".", "/", "!", "$"]))
    async def pu_command(client, message):
        """Handle /pu command"""
        ...
        can_access, reason = await check_gateway_access(user_id, 'pu')
        if not can_access:
            if reason == "locked":
                return
            elif reason == "premium":
        ...
        if not text:
            resp = f"""〈<a href='tg://user?id={user_id}'>꫟</a>〉-» 𝗣𝗮𝘆𝗨 𝗔𝗣𝗜 - CHECK
...
<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /pu cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "PayU API", message):
            return
        
        card, month, year, cvv = card_info
        
        # Forward to admin if enabled
        await forward_to_admin_if_enabled(client, message, 'pu')
        
        # Send progress message
        progress_msg = await message.reply_text("𝙋𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩...")
        
        # Process the card using PayUGateway
        gateway = PayUGateway()
        api_response, error, execution_time = await gateway.check_card(card, month, year, cvv)
        
        # Increment check counter
        db.increment_check(user_id, 'pu')
        ...
        if error:
            ...
            response_text = format_payu_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_payu_response(api_response)
    ...
    print("✅ PayU Gateway commands registered")
