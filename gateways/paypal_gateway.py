...
def categorize_paypal_response(api_response):
    """Categorize PayPal API response"""
    try:
...
def format_paypal_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format PayPal API response for Telegram"""
...
class PayPalGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/paypal/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through PayPal API"""
...
def setup(app: Client):
    ...
    @app.on_message(filters.command(["pp"], prefixes=[".", "/", '!', '$']))
    async def pp_command(client, message):
        """Handle /pp command"""
        ...
        can_access, reason = await check_gateway_access(user_id, 'pp')
        if not can_access:
            if reason == "locked":
                return
            elif reason == "premium":
        ...
        if not text:
            resp = f"""〈<a href='tg://user?id={user_id}'>꫟</a>〉-» 𝗣𝗮𝘆𝗣𝗮𝗹 𝗔𝗣𝗜 - CHECK

〈♻️〉𝙂𝙖𝙩𝙚𝙬𝙖𝙮 -» PayPal API
...
<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /pp cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "PayPal API", message):
            return
        
        card, month, year, cvv = card_info
        
        # Forward to admin if enabled
        await forward_to_admin_if_enabled(client, message, 'pp')
        
        # Send progress message
        progress_msg = await message.reply_text("𝙋𝙡𝙚𝙖𝙨𝙚 𝙒𝙖𝙞𝙩...")
        
        # Process the card using PayPal Gateway
        gateway = PayPalGateway()
        api_response, error, execution_time = await gateway.check_card(card, month, year, cvv)
        
        # Increment check counter
        db.increment_check(user_id, 'pp')
        ...
        if error:
            ...
            response_text = format_paypal_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_paypal_response(api_response)
    ...
    print("✅ PayPal Gateway commands registered")
