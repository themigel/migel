...
PLUGIN_INFO = {
    'name': 'Adyen Gateway',
    'commands': ['ad'], # Changed command to 'ad' for Adyen
    'prefixes': ['.', '/', '!', '$'],
    'description': 'Adyen API payment gateway',
    'type': 'check',
    'status': 'active'
}

def categorize_adyen_response(api_response):
    """Categorize Adyen API response"""
    try:
...
def format_adyen_response(card_info, response_message, suffix, bin_info, execution_time, category):
    """Format Adyen API response for Telegram"""
...
class AdyenGateway:
    def __init__(self):
        self.base_url = "https://onyxenvbot.up.railway.app/adyen/key=yashikaaa/cc="

    async def check_card(self, card, month, year, cvv):
        """Main function to check card through Adyen API"""
...
def setup(app: Client):
    ...
    async def ad_command(client, message):
        ...
        if not text:
            resp = f"""〈<a href='tg://user?id={user_id}'>꫟</a>〉-» 𝗔𝗱𝘆𝗲𝗻 𝗔𝗣𝗜 - CHECK
...
<a href='tg://user?id={user_id}'>╰┈➤</a> 𝙁𝙤𝙧𝙢𝙖𝙩 -» /ad cc|month|year|cvc"""
        ...
        if not await validate_card_with_luhn(card_info, "Adyen API", message):
            return
        ...
        # Process the card using Adyen Gateway
        ...
        if error:
            ...
            response_text = format_adyen_response(
            ...
            # Parse API response
            category, response_message, suffix = categorize_adyen_response(api_response)
